...
public class EscalatedModel : PageModel
{
    ...
    public EscalatedModel(IRequestService requestService)
    {
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        var all = (await _requestService.GetEscalatedRequestsAsync())
            .Where(r => r.EscalatedTo == userId.Value)
            .ToList();
    ...
    public async Task<IActionResult> OnPostUpdateStatusAsync(int requestId, RequestStatus newStatus)
    {
        ...
        if (request.EscalatedTo != userId.Value) return Forbid();

        if (request.Status is RequestStatus.Completed or RequestStatus.Cancelled or RequestStatus.Rejected)
        {
            TempData["ErrorMessage"] = "Yêu cầu này đã được đóng, không thể cập nhật trạng thái.";
